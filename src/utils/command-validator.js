 ...
 */
var CommandValidator = /** @class */ (function () {
     ...
     */
    CommandValidator.validate = function (command) {
        ...
        var cmdLower = cmdTrimmed.toLowerCase();
        ...
        if (this.DANGEROUS_PATTERNS.some(function (pattern) { return pattern.test(cmdLower); })) {
            throw new Error("Dangerous command blocked: ".concat(command));
        }
        ...
        if (this.FORBIDDEN_TOKENS.some(function (token) { return cmdLower.includes(token); })) {
            throw new Error("Command contains unsafe token and was blocked: ".concat(command));
        }
        // Check for forbidden patterns (regex match for flexible whitespace)
        if (this.FORBIDDEN_PATTERNS.some(function (pattern) { return pattern.test(cmdLower); })) {
            throw new Error("Command contains unsafe pattern and was blocked: ".concat(command));
        }
     ...
     */
    CommandValidator.isLikelyInvalid = function (command) {
        var cmdTrimmed = command.trim();
        return this.INVALID_PATTERNS.some(function (pattern) { return pattern.test(cmdTrimmed); });
    };
    /**
     ...
     */
    CommandValidator.getPriority = function (command) {
     ...
     */
    CommandValidator.DANGEROUS_COMMANDS = [
     ...
     */
    CommandValidator.FORBIDDEN_TOKENS = [
     ...
     */
    CommandValidator.FORBIDDEN_PATTERNS = [
     ...
     */
    CommandValidator.INVALID_PATTERNS = [
        /^\d+$/, // Just numbers
        /^invalid_command/i,
     ...
     */
    CommandValidator.DANGEROUS_PATTERNS = CommandValidator.DANGEROUS_COMMANDS.map(function (cmd) { return new RegExp("(?:^|\\s)".concat(cmd.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), "(?:\\s|$)"), 'i'); });
