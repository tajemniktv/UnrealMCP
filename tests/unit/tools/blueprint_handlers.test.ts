...
describe('Blueprint Handlers', () => {
  ...
  it('derives graph connections from get_graph_details', async () => {
      const tools = mockTools as any;
      tools.automationBridge.sendAutomationRequest.mockResolvedValue({
          success: true,
          graphName: 'EventGraph',
  ...
  it('aggregates topology across nested graphs when includeSubGraphs is enabled', async () => {
      const tools = mockTools as any;
      tools.automationBridge.sendAutomationRequest.mockImplementation(async (_toolName: string, args: Record<string, unknown>) => {
